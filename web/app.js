...
function applyTheme(theme) {
    ...
    const meta = document.querySelector('meta[name="theme-color"][media="(prefers-color-scheme: ' + theme + ')"]');
    if (meta) {
        document.querySelectorAll('meta[name="theme-color"]').forEach(m => m.removeAttribute('media'));
...
function updateThemeToggleIcon(theme) {
    ...
    if (theme === 'dark') {
        ...
        btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg> Dark';
    }
}

// System theme change → follow (case 1: last-event wins)
